...
int main(int argc, char *argv[])
{
	...
	if (argc != 4) {
		fprintf(stderr, "Usage: %s inventory_file1 inventory_file2 merged_file\n", argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	while (num_read1 == 1 && num_read2 == 1) {
		if (part1.number < part2.number) {
			...
			if (strcmp(part1.name, part2.name) != 0)
				fprintf(stderr, "Error: part %d names do not match. Using name %s\n", part1.number, part1.name);
	...
	while (num_read1 == 1) {
		fwrite(&part1, sizeof(struct part), 1, fp_out);
		num_read1 = fread(&part1, sizeof(struct part), 1, fp_out);
	}
	
	while (num_read2 == 1) {
		fwrite(&part2, sizeof(struct part), 1, fp_out);
		num_read2 = fread(&part2, sizeof(struct part), 1, fp_out);
	}
